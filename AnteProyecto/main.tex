...
Para investigar cómo se manifiestan estas propiedades en el régimen cuántico, las caminatas cuánticas \janote{discretas en el tiempo (DTQWs por sus siglas en inglés)} ofrecen un marco de estudio ventajoso. En estos modelos discretos es posible imponer las geometrías de los billares clásicos directamente como condiciones de frontera\janote{esta no es ventaja, en la partícula libre también se puede hacer}. Esto resulta ideal ya que las caminatas cuánticas\janote{DTQWs especificamente,no cualquier moderlo de caminata cuántica (por eso un comentario mío arriba)} nos permiten variar de manera independiente tanto estas condiciones de frontera como el operador de moneda, ofreciendo así un excelente escenario para estudiar a detalle la dinámica del sistema\janote{esta última parte del enunciado cambiala para decir que nos permite estudiar si sólo las condiciones de frontera dan lugar a caos}.
...
dichos trabajos se han restringido al uso de un solo qubit\janote{sistema de dos niveles en vez de qubit} como estado\janote{es un subsistema} de moneda~\cite{alonso-lobo2025simplest}. Esta limitación presenta dos problemas importantes. 
Por un lado, las distribuciones espectrales obtenidas bajo este enfoque no logran\janote{no concuerdan más bien} reproducir con exactitud las predicciones universales del Ensamble Ortogonal Gaussiano (GOE) esperadas para sistemas caóticos. 
...
emplear un sistema de dos niveles (qubit) para gobernar esta dinámica resulta estructuralmente restrictivo y artificial. Resulta mucho más natural y directo codificar estos cuatro movimientos ortogonales en un sistema de moneda que posea la misma dimensionalidad, es decir, un estado de cuatro dimensiones (qudit). Por lo tanto, es de interés investigar si esta formulación mejora\janote{rigurosamente no es mejorar, sino que simplementen se sigan las predicciones esperadas} las firmas espectrales y permite determinar si la topología de las fronteras es una condición suficiente para dictar, por sí sola, el caos o la integrabilidad del sistema.
...
En primer lugar, se implementará numéricamente el operador de evolución
unitaria dado por la ecuación~\eqref{eq:U}. Para ello, se definirán
computacionalmente los espacios de posición acotados por billares con
geometrías de rectángulo y Sinaí. La estructura del operador de desplazamiento
$S$ se establecerá mediante reglas que condicionan el movimiento del caminante
hacia sus primeros vecinos siguiendo un esquema de cruz (es decir, permitiendo
únicamente transiciones a lo largo de los ejes cartesianos de la red
bidimensional), según el estado de la moneda. Por su parte, la dinámica interna
del caminante se gobernará mediante el operador $C = \mathbb{I}_P \otimes
\mathcal{C}$, donde $\mathbb{I}_P$ representa el operador identidad en el
espacio de posición y $\mathcal{C}$ es un operador unitario que actúa
exclusivamente sobre el espacio de la moneda de cuatro dimensiones. Cabe
destacar que la implementación numérica de \eqref{eq:U} y las herramientas de
análisis no se programarán desde cero, sino que se llevarán a cabo utilizando
librerías especializadas desarrolladas en Wolfram Mathematica\janote{Empezaste el párrafo diciendo que vas a implementar y terminaste diciendo que más bien vas a usar herramientas de libs. Arreglalo para ser consistente en todo el párrafo}. 
