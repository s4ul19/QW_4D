...
\sout{En primer lugar, se implementará numéricamente el operador de evolución unitaria dado por la ecuación}~\eqref{eq:U}\sout{.}
\janote{Empezaste el párrafo diciendo que vas a implementar y terminaste diciendo que más bien vas a usar herramientas de libs. Arreglalo para ser consistente en todo el párrafo}
\snnote{Contradicción resuelta. Se estableció desde la primera oración que todo el modelo se construirá utilizando las librerías, eliminando la aclaración del final.} 
En primer lugar, la construcción computacional del operador de evolución unitaria dado por la ecuación~\eqref{eq:U} y de las herramientas de análisis se llevará a cabo utilizando librerías especializadas desarrolladas en Wolfram Mathematica.
Para ello, se definirán los espacios de posición acotados por billares con geometrías de rectángulo y Sinaí. 
...
\sout{Cabe destacar que la implementación numérica de}~\eqref{eq:U}
\sout{y las herramientas de análisis no se programarán desde cero, sino que se llevarán a cabo utilizando librerías especializadas desarrolladas en Wolfram Mathematica.}
...
\janote{La posición de esta sección es rígida? Si no, mover a alguna posción después de Marco teórico}. 
\snnote{He movido la sección, no creo que sea rígida la posición.}
...
Para investigar cómo se manifiestan estas propiedades en el régimen cuántico, las caminatas cuánticas discretas en el tiempo (DTQWs por sus siglas en inglés)
 \janote{discretas en el tiempo (DTQWs por sus siglas en inglés)}\snnote{Agregado.} 
 ofrecen un marco de estudio ventajoso,
\sout{En estos modelos discretos es posible imponer las geometrías de los billares clásicos directamente como condiciones de frontera}
\janote{esta no es ventaja, en la partícula libre también se puede hacer}\snnote{He reescrito esta parte}.
\sout{Esto resulta ideal} ya que nos permiten variar de manera independiente tanto las condiciones de frontera como el operador de moneda, \sout{ofreciendo así un excelente escenario para estudiar a detalle la dinámica del sistema}\janote{esta última parte del enunciado cambiala para decir que nos permite estudiar si sólo las condiciones de frontera dan lugar a caos}\snnote{Reescrito.} ofreciendo así las herramientas necesarias para determinar si la topología de las fronteras es, por sí sola, suficiente para dar lugar a la emergencia del caos.
...
dichos trabajos se han restringido al uso de un solo \sout{qubit}\janote{sistema de dos niveles en vez de qubit}\snnote{Término sustituido.} sistema de dos niveles como \sout{estado}\janote{es un subsistema}\snnote{Corregido.} subsistema de moneda~\cite{alonso-lobo2025simplest}. Esta limitación presenta dos problemas importantes. 
Por un lado, las distribuciones espectrales obtenidas bajo este enfoque \sout{no logran reproducir con exactitud}\janote{no concuerdan más bien}\snnote{Arreglado.} no concuerdan exactamente con las predicciones universales del Ensamble Ortogonal Gaussiano (GOE) esperadas para sistemas caóticos. 
...
emplear un sistema de dos niveles (qubit) para gobernar esta dinámica resulta estructuralmente restrictivo y artificial. Resulta mucho más natural y directo codificar estos cuatro movimientos ortogonales en un sistema de moneda que posea la misma dimensionalidad, es decir, un estado de cuatro dimensiones (qudit). Por lo tanto, es de interés investigar si esta formulación \sout{mejora las firmas espectrales y permite}\janote{rigurosamente no es mejorar, sino que simplementen se sigan las predicciones esperadas}\snnote{Reescrito.} permite que el comportamiento espectral siga las predicciones estadísticas esperadas, permitiendo determinar si la topología de las fronteras es una condición suficiente para dictar, por sí sola, el caos o la integrabilidad del sistema.
